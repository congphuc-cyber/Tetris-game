...
def draw_grid(surface, grid):
    ...
    # Vẽ các ô trước (các khối đã khóa hoặc khối đang rơi)
    for i in range(len(grid)):
        for j in range(len(grid[i])):
            pygame.draw.rect(surface, grid[i][j], (sx + j * BLOCK_SIZE, sy + i * BLOCK_SIZE, BLOCK_SIZE, BLOCK_SIZE), 0)

    # Vẽ các đường lưới sau để chúng hiển thị trên các ô
    for i in range(len(grid) + 1):  # Thêm 1 để vẽ cả đường biên cuối
        pygame.draw.line(surface, GRID_COLOR, (sx, sy + i * BLOCK_SIZE), (sx + PLAY_WIDTH, sy + i * BLOCK_SIZE))
    for j in range(len(grid[0]) + 1):  # Thêm 1 để vẽ cả đường biên cuối
...
def menu_selection(win):
    ...
    while run:
        ...
        draw_text(win, "TETRIS", SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 200, 85, RED)
        ...
        draw_button(win, "Thoát", exit_button.x, exit_button.y, exit_button.width, exit_button.height, BRIGHT_BLUE)
